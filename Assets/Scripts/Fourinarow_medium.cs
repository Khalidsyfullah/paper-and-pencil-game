...
public class Fourinarow_medium : MonoBehaviour
{
    ...
    GameObject[,] grid_cell = new GameObject[8, 6];
    int[,] grid_board = new int[8, 6];
    Vector2Int[] emptyCells = new Vector2Int[6];
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            emptyCells[i] = new Vector2Int(7, i);
            SpriteRenderer spriteRenderer = grid_cell[7, i].GetComponent<SpriteRenderer>();
            spriteRenderer.sprite = possible_move;
        }


        settings = PlayerPrefs.GetInt("fourinarowmedium", 0);
